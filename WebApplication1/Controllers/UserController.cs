...
namespace WebApplication1.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult SignUp(tbl_user uvm, HttpPostedFileBase imgfile)
        {
            ...
            if (path.Equals("-1"))
            {
                ViewBag.error = "Resim yüklenemiyor!";
            }
            else
            {
        ...
        public ActionResult login(tbl_user avm)
        {
            ...
            {
                ViewBag.error = "Wrong username or password has been entered!";
        ...
        if (path.Equals("-1"))
        {
            ViewBag.error = "The image can't be uploaded!";
        }
        else
        {
        ...
        public string uploadimgfile(HttpPostedFileBase file)
        {
            ...
            if (file != null && file.ContentLength > 0)
            {
                ...
                if (extension.ToLower().Equals(".jpg") || extension.ToLower().Equals(".jpeg") || extension.ToLower().Equals(".png"))
                {
                    try
                    {
                        ...
                        path = "~/Content/upload/" + random + Path.GetFileName(file.FileName);
                         ...
                         ViewBag.Message = "The File has been succesfully uploaded!"; //BURASI İSTEĞE BAĞLI KALDIRILABİLİR
                    }
                    catch (Exception ex)
                    {
                        path = "-1";
                ...
                {
                    Response.Write("<script>alert('Only jpg, jpeg and png formats can be acceptable!'); </script>");
                }
            }
